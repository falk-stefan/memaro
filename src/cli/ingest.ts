...
const indexChunks = async (doc: InstructionDocEntity): Promise<void> => {
  ...
  const points = await Promise.all(
    chunks.map(async (chunk, index) => ({
      id: randomUUID(),
      vector: {
        dense: await embedDocument(chunk.embeddingText),
        sparse: sparseVector(chunk.embeddingText),
      },
      payload: {
        docId: Number(doc.id),
        chunkIndex: index,
...
async function syncSource(sourceName: string): Promise<void> {
  ...
  console.log(
    `[${source.id}] created=${created} updated=${updated} unchanged=${unchanged} removed=${removed}`,
  );
}
